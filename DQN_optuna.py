...
import box
import os, shutil, sys, argparse, optuna
...
parser.add_argument('--update', type=str, default='soft', help="hard or soft ['hard', soft']")
parser.add_argument('--save', type=str, default='OPTUNA-DQN', help='name of the file')
...
def objective(trial):
    ...
    network_params = {
        ...
        'hidden_dim': trial.suggest_int("hidden_dim", 20, 200)              #64
    }

    training_params = {
        'batch_size': 1,                                                    # 256
        'gamma': 0.95,          #trial.suggest_float("gamma", 0.1, 0.99),    # 0.95,
        'epsilon_start': 1.1,
        'epsilon_end': trial.suggest_float("epsilon_end", 0.0001, 0.05),     # 0.05,
        'epsilon_decay': trial.suggest_float("epsilon_decay", 0.1, 0.99),    # 0.95,
        'target_update': 'tmp',                                             # use 'soft' or 'hard'
        'tau': 0,                                       # 0.01,             # relevant for soft update
        'target_update_period': 1,                      # 15                # relevant for hard update
        'grad_clip': trial.suggest_float("grad_clip", 0.05, 0.3),  # 0.1,
    }
    network_params = box.Box(network_params)
    params = box.Box(training_params)

    # Target Network
    if args.with_target:    # with target network
        if args.update == 'soft':
            params.target_update = 'soft'
            params.tau = trial.suggest_float("tau", 0.001, 0.99)                           # relevant for soft update
        else:
            params.target_update = 'hard'
            params.target_update_period = trial.suggest_int("target_update_period", 5, 50) # relevant for hard update
    else:       # without target network
        params.target_update = 'hard'
    ...
    if args.with_buffer:  # with Replay Buffer
        ...
        params.batch_size = trial.suggest_int("batch_size", 50, 500)  # 256
    ...
    for i_episode in range(max_episodes):
        ...
        for t in count():
            ...
            if done or t >= max_score:
                ...
                trial.report(score, i_episode)  # reporting optuna the score
                break
...
# Steps are reported as 5, 10 ... but pruner is looking at steps from 0,1...
pruner = optuna.pruners.PercentilePruner(25.0, n_startup_trials=5, n_warmup_steps=1, interval_steps=1)
study = optuna.create_study(study_name="DQN", direction="maximize", sampler=sampler, pruner=pruner)
study.optimize(objective, n_trials=args.trials, catch=(RuntimeError,),  gc_after_trial=True)   # , timeout=260_000)
...
if len(complete_trials):
    trial = study.best_trial
    logging.info("Score: {}".format(trial.value))
    logging.info("Params: ")
    for key, value in trial.params.items():
        logging.info("    {}: {}".format(key, value))

    fig = optuna.visualization.plot_param_importances(study)
    fig.write_image(os.path.join(save, 'optuna_param_importances.png'))
    fig.write_html(os.path.join(save, 'optuna_param_importances.html'))
    fig = optuna.visualization.plot_intermediate_values(study)
    fig.write_image(os.path.join(save, 'optuna_intermediate_values.png'))
    fig.write_html(os.path.join(save, 'optuna_intermediate_values.html'))
